...
pub async fn fetch_dataset_link_check_results_v2(configuration: &configuration::Configuration, ) -> Result<models::FetchDatasetLinkCheckResultsV2200Response, Error<FetchDatasetLinkCheckResultsV2Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FetchDatasetLinkCheckResultsV2200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FetchDatasetLinkCheckResultsV2200Response`")))),
        }
    } else {
        let content = resp.text().await?;
