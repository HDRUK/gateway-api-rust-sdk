...
/// struct for typed errors of method [`integrations_datasets_test`]
...
pub async fn integrations_datasets_test(configuration: &configuration::Configuration, datasets_test_request: models::DatasetsTestRequest) -> Result<models::CreateDarIntegration201Response, Error<IntegrationsDatasetsTestError>> {
    ...
    let uri_str = format!("{}/api/v1/integrations/datasets/test", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<IntegrationsDatasetsTestError> = serde_json::from_str(&content).ok();
