...
/// struct for typed errors of method [`edit_reviews`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`update_reviews`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn delete_reviews(configuration: &configuration::Configuration, id: i32) -> Result<models::DeleteAliases200Response, Error<DeleteReviewsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteReviewsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn edit_reviews(configuration: &configuration::Configuration, id: i32, create_reviews_request: models::CreateReviewsRequest) -> Result<models::UpdateReviews200Response, Error<EditReviewsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::UpdateReviews200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::UpdateReviews200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<EditReviewsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn update_reviews(configuration: &configuration::Configuration, id: i32, create_reviews_request: models::CreateReviewsRequest) -> Result<models::UpdateReviews200Response, Error<UpdateReviewsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateReviewsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
